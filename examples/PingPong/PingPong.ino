	...
	if (isMaster == true)
	{
		if (BufferSize > 0)
		{
			if (strncmp((const char *)RcvBuffer, (const char *)PongMsg, 4) == 0)
			{
				...
				cadTime = millis();
				Radio.StartCad();
				// Correct behaviour here would be to wait for the CAD callback and if the channel is available send the response.
				// To keep the example simple, we just wait 500ms here to see the result of CAD and send nethertheless
				delay(500);
...
/**@brief Fucntion to switch the LoRa antenna of an eByte E22 module to receive mode
 */
void enableRX(void)
{
...
/**@brief Fucntion to switch the LoRa antenna of an eByte E22 module to transmit mode
 */
void enableTX(void)
{
